...
namespace TCAMultiplayer
{
    ...
    public class PluginRunner : MonoBehaviour
    {
        ...
        private void HandleAircraftStateRaw(ulong fromPeerId, byte[] data)
        {
            ...
            // Host relay: broadcast state to all other clients over unreliable transport.
            // Unreliable is the correct choice for continuous 60Hz position updates — if a packet drops,
            // the next one arrives 16ms later. Combat packets (gun, missile) use reliable since each
            // event is critical and infrequent.
            if (_activeSession != null && _activeSession.IsHost && fromPeerId != _activeSession.LocalPeerId)
            {
